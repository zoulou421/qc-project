...
class WorkProgram(models.Model):
    ...
    def _onchange_project_id(self):
        """Filtrer les activités basées sur le domaine/projet sélectionné."""
        if self.project_id:
            # Recherche des processus liés au domaine
            process_ids = self.env['workflow.process'].search([('domain_id', '=', self.project_id.id)])
            # Recherche des sous-processus liés aux processus
            sub_process_ids = self.env['workflow.subprocess'].search([('process_id', 'in', process_ids.ids)])
            # Filtrer les activités liées aux sous-processus
            self.activity_id = False
            return {'domain': {'activity_id': [('sub_process_id', 'in', sub_process_ids.ids)]}}
        else:
            self.activity_id = False
    ...
    def _onchange_activity_id(self):
        """Filtrer les procédures et livrables basés sur l'activité sélectionnée."""
        if self.activity_id:
            return {
                'domain': {
    ...
    def _onchange_procedure_id(self):
        """Filtrer les descriptions de tâches basées sur la procédure sélectionnée."""
    ...
    def import_work_program(self, row):
        """Méthode pour importer une ligne de données du programme de travail."""
        try:
            vals = {
                ...
                'comments': row.get('Comments / Remarks / Problems encountered / Additionals informations')
            }

            # Recherche ou création des enregistrements liés
            if row.get('Project / WP'):
                project = self.env['workflow.domain'].search([('name', '=', row['Project / WP'])], limit=1)
                if project:
